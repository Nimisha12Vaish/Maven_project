...
public class Test_BlanketMicroflow {
	...
	public void f_BlanketMicroflow() throws InterruptedException{
		...
		//Click on Submit button
		WebElement myDynamicElement = (new WebDriverWait(driver, 10)).until(ExpectedConditions.presenceOfElementLocated(By.xpath(prop1.getProperty("w3_t_summary_submit"))));
		...
		*/
		...
		System.out.println("submit button submitted");
		...
		for(int i=0; i<4;i++) 
			try
		{ 				
		owait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(prop1.getProperty("t_continue"))));
		WebElement submit_button = driver.findElement(By.xpath(prop1.getProperty("t_continue")));
		
		String text = submit_button.getAttribute("id");
		...
		//click on Submit button
		JavascriptExecutor js = (JavascriptExecutor) driver;
		WebElement element_submit = driver.findElement(By.id("com_ibm_ciolab_core_Button_9_label"));
		js.executeScript("arguments[0].scrollIntoView(false);", element_submit);
		
		System.out.println(element_submit.getAttribute("id"));
